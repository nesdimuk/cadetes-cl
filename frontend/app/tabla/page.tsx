...
import { StandingEntry, StandingsData } from "@/lib/types"
...
function TablaInner() {
  ...
  const groups = useMemo(() => {
    if (!data) return {}
    const catRows = data.standings.filter(s => s.category === selectedCategory)
    const grouped: Record<string, StandingEntry[]> = {}
    for (const row of catRows) {
      if (!grouped[row.group]) grouped[row.group] = []
      grouped[row.group].push(row)
    }
    // Ordenar por rank dentro de cada grupo
